...
class SimpleVirus(object):
    ...
    def reproduce(self, popDensity):
        ...
        # TODO
        prob = random.random()
        reProX2= self.maxBirthProb * ( 1 - popDensity )
        if prob < reProX2:
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    # TODO
    finalResults = np.array([ 0 for _ in range(300)])
    for _ in range(numTrials):
        finalResults += runSimulation(numViruses, maxPop, maxBirthProb, clearProb)
    finalResults =  finalResults / numTrials
...
def runSimulation(numViruses, maxPop, maxBirthProb, clearProb):
    '''helper fuction for doing one simulation run'''
    viruses = [ SimpleVirus(maxBirthProb, clearProb) for _ in range(numViruses)]
    patient = Patient(viruses , maxPop)
    numSteps = 300
    numVirusesEachStep = [patient.update() for i in range (numSteps)]
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantToAll(self, drugList):
        return bool(0 not in [0 for drug in drugList if not self.resistances[drug]])
...
class TreatedPatient(Patient):
    ...
    def update(self):
        ...
        # TODO
        survivedViruses= [virus for virus in self.viruses if not virus.doesClear()]
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    # TODO
    finalResults = np.array([0 for _ in range(600)])
    for _ in range(numTrials):
        finalResults += runDrugSimulation(numViruses, maxPop, maxBirthProb, clearProb, resistances, mutProb, 150, 300)
    finalResults =  finalResults / numTrials
    totalViruses , resistantViruses = np.array_split(finalResults, 2)
...
def runDrugSimulation(numViruses, maxPop, maxBirthProb, clearProb, resistances, mutProb, numStepsBeforeDrugApplied, totalNumSteps):
    ...
    if numStepsBeforeDrugApplied > totalNumSteps:
        raise AssertionError
    viruses = [ResistantVirus(maxBirthProb, clearProb, resistances, mutProb) for _ in range(numViruses)]
    patient = TreatedPatient(viruses, maxPop)
    numVirusesEachSteps, numResistantVirusesEachSteps = [], []
    for _ in range(totalNumSteps):
        if _ == numStepsBeforeDrugApplied:
            patient.addPrescription("guttagonol")
        numVirusesEachSteps.append(patient.update())
        numResistantVirusesEachSteps.append(patient.getResistPop(['guttagonol']))
    return numVirusesEachSteps + numResistantVirusesEachSteps
