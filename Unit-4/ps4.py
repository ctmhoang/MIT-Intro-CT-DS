...
#    pass
    mean = sum(y)/len(y)
    return 1 - (sum(list(map(lambda yi, ei : (yi -ei)**2 , y, estimated))) / sum([(yi - mean)**2 for yi in y]) )
...
#    pass
    ...
    for _ in models:
        ...
        pylab.title("Templature vs time")
        pylab.xlabel("Year")
        pylab.ylabel("Templature")
        estYVals = np.polyval(_,xVals)
        pylab.plot(xVals, estYVals, 'r-', label = 'R^2 = ' + str(r_squared(y,estYVals)), linewidth = 1.7 )
